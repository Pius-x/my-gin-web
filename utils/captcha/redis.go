...
func (rs *RedisStore) Set(id string, value string) {
	...
	if err != nil {
		global.ZapLog.Error("RedisStoreSetError!", zap.Error(err))
	}
}
...
func (rs *RedisStore) Get(key string, clear bool) string {
	...
	if err != nil {
		global.ZapLog.Error("RedisStoreGetError!", zap.Error(err))
		return ""
	}
	if clear {
		err := global.Redis.Del(rs.Context, key).Err()
		if err != nil {
			global.ZapLog.Error("RedisStoreClearError!", zap.Error(err))
			return ""
		}
	}
	return val
